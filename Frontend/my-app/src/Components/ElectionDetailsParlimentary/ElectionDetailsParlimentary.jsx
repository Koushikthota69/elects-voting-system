...
import './ElectionDetailsParlimentary.css'; // Unified CSS
...
const ElectionDetailsParlimentary = () => {
    ...
    const fetchElectionData = async () => {
        try {
            console.log('🔍 Fetching parliamentary election data for ID:', id);

            const [electionResponse, candidatesResponse] = await Promise.all([
                axios.get(API_ENDPOINTS.PARLIAMENTARY_ELECTION_DETAILS(id)),
                axios.get(API_ENDPOINTS.CANDIDATES)
            ]);

            const electionData = electionResponse.data.data;
            console.log('📊 Parliamentary election data:', electionData);
            ...
            console.error('❌ Error fetching parliamentary election data:', err);
                    ...
                    try {
                        ...
                        if (verificationResponse.data.success) {
                            ...
                            const voteResponse = await axios.post(
                                API_ENDPOINTS.VOTE_PARLIAMENTARY_ELECTION(election._id, candidateId),
                                {
                                    voterId: userId,
    ...
    if (loading) {
        return (
            <div className="election-loading">
                ...
                <p>Loading parliamentary election details...</p>
            </div>
        );
    }
    ...
    if (!election) {
        return (
            <div className="election-error">
                ...
                <p>The parliamentary election you're looking for doesn't exist.</p>
            </div>
        );
    }
    ...
    return (
        <div className={`election-details-container ${theme}`}>
            ...
            <div className="election-header">
                <h2 className={`election-title ${theme}`}>Parliamentary Election - {election.year}</h2>
