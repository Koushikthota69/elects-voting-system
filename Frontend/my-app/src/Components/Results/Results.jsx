...
const Results = () => {
      ...
      const fetchElectionDetails = async () => {
        try {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.response?.data?.message || 'Failed to fetch election results. Please try again.'
          })
        } finally {
          setLoading(false)
        }
  ...
  const handleElectionChange = event => {
    ...
    if (selectedElection) {
      ...
      if (status === 'Ongoing') {
        Swal.fire('Notice', 'The Election is still ongoing. Results will be available after it finishes.', 'info').then(() => {
          navigate('/')
        })
        return
      }
  ...
  const calculateTotalVotes = () => {
    ...
    return electionDetails.results.totalVotes || 0
  }
  ...
  const findWinner = () => {
    ...
    if (sortedCandidates.length === 0 || sortedCandidates[0].votes === 0) return 'No winner'

    const highestVotes = sortedCandidates[0].votes
    const winners = sortedCandidates.filter(candidate => candidate.votes === highestVotes)

    if (winners.length > 1) return `${winners.length} Winners (Tie)`
  ...
  const pieChartData = {
    labels: voteDistribution.map(item => item.candidateId?.user?.firstName || 'Unknown Candidate'),
    datasets: [{ data: voteDistribution.map(item => item.votes || 0), backgroundColor: COLORS }]
  }
  ...
  return (
    <div
      className={isBlurred ? `blur-background ${theme}` : `results-container ${theme}`}
      id='resultsSection'
    >
      ...
      <div className={`form-container ${theme}`}>
        ...
        <div className={`radio-buttons ${theme}`}>
          <label>
            <input type='radio' name='election-type' value='general' onChange={handleElectionTypeChange} checked={electionType === 'general'} />
            General Election
          </label>
          <label>
            <input type='radio' name='election-type' value='presidential' onChange={handleElectionTypeChange} checked={electionType === 'presidential'} />
            Presidential Election
          </label>
          <label>
            <input type='radio' name='election-type' value='parlimentary' onChange={handleElectionTypeChange} checked={electionType === 'parlimentary'} />
            Parliamentary Election
          </label>
          <label>
            <input type='radio' name='election-type' value='provincial' onChange={handleElectionTypeChange} checked={electionType === 'provincial'} />
            Provincial Election
          </label>
        </div>
              ...
              {elections.map(election => {
                ...
                return (
                  <option key={election._id} value={election._id}>
                    {`${electionType === 'general' ? election.name : `${election.year} ${election.province || ''}`} - ${status}`}
                  </option>
                )
              })}
            </select>
      ...
      {electionDetails && (
        <div className={`results-details ${theme}`}>
          ...
          <div className="download-section">
            <button onClick={downloadResultsPDF} className={`download-btn ${theme}`} title="Download Results as PDF">
              <FaDownload size={20} />
                  ...
                  {[...voteDistribution]
                    ...
                    .map((item, index) => (
                      <div key={index} className={`candidate-card ${theme}`}>
                        ...
                        <img
                          src={item.candidateId?.user?.profilePhoto || unavailable}
                        ...
                        <div className={`candidate-info ${theme}`}>
                          <h4>{item.candidateId?.user?.firstName || 'Unknown'} {item.candidateId?.user?.lastName || ''}</h4>
                          <p className="candidate-votes">Votes: {(item.votes || 0).toLocaleString()}</p>
                          {item.candidateId?.party && (
                            <p className="candidate-party">Party: {item.candidateId.party.name}</p>
                          )}
                          {item.candidateId?.user ? (
                            <Link to={`/candidate/${item.candidateId.user._id}`} className='candidate-link'>View Details</Link>
                          ) : (
