...
const ElectionDetailsPresidential = () => {
  ...
  const fetchElectionData = async () => {
    try {
      console.log('🔍 Fetching presidential election data for ID:', id);

      const [electionResponse, candidatesResponse] = await Promise.all([
        axios.get(API_ENDPOINTS.PRESIDENTIAL_ELECTION_DETAILS(id)),
        axios.get(API_ENDPOINTS.CANDIDATES)
      ]);

      const electionData = electionResponse.data.data || electionResponse.data.election || electionResponse.data;
      console.log('📊 Presidential election data:', electionData);
      ...
      const candidatesData = candidatesResponse.data.data || candidatesResponse.data.candidates || candidatesResponse.data || [];
      setCandidates(candidatesData);

      // Check if user has voted
      const userId = localStorage.getItem('user-id');
      const results = electionData.results || {};
      const voteDistribution = results.voteDistribution || [];

      if (userId && voteDistribution.length > 0) {
        const votedCandidate = voteDistribution.find(candidate =>
      ...
      console.error('❌ Error fetching presidential election data:', err);
  ...
  // ✅ IMPROVED: Enhanced voting function with better face verification
  const handleVote = async (candidate, candidateId) => {
    console.log('🗳️ Attempting to vote for presidential candidate:', candidateId);
      ...
      html: `
        <div style="text-align: center;">
          ...
          <p style="font-size: 1.2em; font-weight: bold; color: #007bff; margin: 10px 0;">
            ${candidate.user?.firstName || candidate.name || 'Unknown'} ${candidate.user?.lastName || ''}
          </p>
          ...
          try {
            ...
            if (verificationResponse.data.success) {
              ...
              const voteResponse = await axios.post(
                API_ENDPOINTS.VOTE_PRESIDENTIAL_ELECTION(election._id, candidateId),
                {
                  voterId: userId,
          ...
          html: `
            <div style="text-align: center;">
              ...
              <p style="font-size: 0.9em; color: #666;">
                Your vote for <strong>${candidate.user?.firstName || candidate.name || 'Unknown'} ${candidate.user?.lastName || ''}</strong> has been recorded securely.
              </p>
            </div>
          `,
          timer: 5000,
  ...
  if (loading) {
    return (
      <div className="election-loading">
        ...
        <p>Loading presidential election details...</p>
      </div>
    );
  }
  ...
  if (!election) {
    return (
      <div className="election-error">
        ...
        <p>The presidential election you're looking for doesn't exist.</p>
      </div>
    );
  }
  ...
  // SAFE: Ensure candidates is always an array
  const electionCandidates = election.candidates || candidates || [];

  return (
    <div className={`election-details-container ${theme}`}>
      {/* Election Header */}
      <div className="election-header">
        <h2 className={`election-title ${theme}`}>Presidential Election - {election.year || 'N/A'}</h2>
      ...
      <div className="election-info">
        ...
        <div className="info-section">
          ...
          <div className={`election-description ${theme}`}>
            {election.rules || 'No rules specified'}
          </div>
        </div>
      </div>
            ...
            <table className={`candidates-table ${theme}`}>
              ...
              <tbody>
                {electionCandidates.map((candidate, index) => (
                  <tr key={candidate._id || index} className="candidate-row">
                    <td className="candidate-index" onClick={() => handleRowClick(candidate.user?._id || candidate._id)}>
                      {index + 1}
                    </td>
                    <td className="candidate-photo" onClick={() => handleRowClick(candidate.user?._id || candidate._id)}>
                      <img
                        className='profile'
                        src={candidate.user?.profilePhoto || candidate.profilePhoto || '/default-profile.png'}
                        alt={candidate.user?.firstName || candidate.name || 'Candidate'}
                        onError={(e) => {
                          e.target.src = '/default-profile.png';
                        }}
                      />
                    </td>
                    <td className="candidate-info" onClick={() => handleRowClick(candidate.user?._id || candidate._id)}>
      ...
      {/* Voting Instructions */}
      {electionStatus === 'active' && !votedCandidateId && (
