...
router.get('/', async (req, res) => {
    try {
        const elections = await PresidentialElection.find().sort({ date: -1 });
        res.status(200).json({ success: true, data: elections });
    } catch (error) {
        console.error('❌ Get presidential elections error:', error);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        console.error('❌ Get presidential election by ID error:', error);
        res.status(500).json({ success: false, message: "Server Error: " + error.message });
    }
});
...
router.get('/election/:id', async (req, res) => {
    try {
        ...
        console.error('❌ Get presidential election with candidates error:', error);
        res.status(500).json({ success: false, message: "Server Error: " + error.message });
    }
});

// ✅ FIXED: Add New Presidential Election with consistent date/time storage
router.post('/', async (req, res) => {
    console.log('📥 Received presidential election creation request:', req.body);
    ...
    try {
        ...
        console.log('✅ Presidential election created successfully:', newElection._id);

        res.status(201).json({
            success: true,
            message: "Presidential election created successfully!",
            data: newElection
        });
    } catch (error) {
        console.error('❌ Presidential election creation error:', error);
...
// ✅ FIXED: Update Presidential Election with consistent date/time storage
router.put('/:id', async (req, res) => {
    const electionId = req.params.id;
    const { year, date, startTime, endTime, description, rules } = req.body;

    console.log('📥 Updating presidential election:', electionId, req.body);

    try {
        // Validate election exists
        const existingElection = await PresidentialElection.findById(electionId);
        if (!existingElection) {
        ...
        console.log('✅ Presidential election updated successfully:', election._id);

        res.status(200).json({
            success: true,
            message: 'Presidential election updated successfully',
            data: election
        });
    } catch (error) {
        console.error('❌ Error updating presidential election:', error);
...
// ✅ FIXED: Apply for Presidential Election
router.post('/:id/apply', async (req, res) => {
    try {
        console.log('🔍 Presidential election apply request received:', req.params.id, req.body);
        ...
        console.log('✅ Candidate applied successfully to presidential election');

        res.status(200).json({
            success: true,
            message: 'Applied successfully to presidential election',
            data: {
        ...
        console.error('❌ Presidential election apply error:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error during election application: ' + error.message
        });
    }
});

// ✅ FIXED: Enhanced voting endpoint for Presidential Election with consistent date/time
router.post('/:id/vote/:candidateId', async (req, res) => {
    const { voterId } = req.body;
    const { candidateId, id: electionId } = req.params;

    console.log('🗳️ Presidential voting request:', { electionId, candidateId, voterId });
    ...
    try {
        ...
        console.log('⏰ Presidential election time check:', {
        ...
        console.log('✅ Vote recorded successfully for presidential election');
        ...
        console.error('❌ Presidential election voting error:', error);
        res.status(500).json({
            success: false,
...
// ✅ FIXED: Get Presidential Election results with consistent date/time
router.get('/:id/results', async (req, res) => {
    try {
        console.log('📊 Fetching presidential election results:', req.params.id);
        ...
        console.log('✅ Presidential election results calculated');
        ...
        console.error('❌ Presidential election results error:', error);
        res.status(500).json({
            success: false,
...
router.delete('/:id', async (req, res) => {
    try {
        await Service.deleteById(req, res, PresidentialElection, name);
    } catch (error) {
        console.error('❌ Delete presidential election error:', error);
        res.status(500).json({
            success: false,
            message: "Server Error: " + error.message
        });
